...
import { renderManageManager } from './manage.js';
...
window.goManager = function(v) {
  withTransition('fade', () => {
    ['district','manage'].forEach(x => $('mnav-' + x)?.classList.toggle('active', x === v));
    ['lview-district','lview-manage-mgr'].forEach(id => $(id)?.classList.remove('active'));
    if (v === 'district') { $('lview-district')?.classList.add('active');    renderDistrict(); }
    if (v === 'manage')   { $('lview-manage-mgr')?.classList.add('active');  renderManageManager(); }
  });
};
